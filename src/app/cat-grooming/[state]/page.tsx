...
import { CaretRight, Cat, MapPin } from "@phosphor-icons/react/dist/ssr";
...
export async function generateMetadata({ params }: StatePageProps): Promise<Metadata> {
  ...
  const title = `Cat Groomers in ${stateName} | Browse by City`;
  const description = `Browse cat groomers across ${stateName}. Find feline-friendly salons with gentle handling in every city.`;

  return {
    title,
    description,
    alternates: { canonical: `/cat-grooming/${state}` },
    openGraph: {
      title,
      description,
      type: "website",
      url: `/cat-grooming/${state}`,
      siteName: "GroomLocal",
      images: [{ url: "/og-image.png", width: 1200, height: 630, alt: `Cat groomers in ${stateName}` }],
    },
    twitter: {
...
export default async function CatGroomingStatePage({ params }: StatePageProps) {
  const { state } = await params;
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="font-heading text-4xl md:text-5xl font-bold text-brand-primary">
              Cat Groomers in <span className="text-brand-secondary">{stateName}</span>
            </h1>
          </div>
          <p className="text-text-muted flex items-center gap-1.5 text-lg">
            <MapPin weight="fill" className="w-5 h-5 text-brand-secondary" />
            Browse {cities.length} cities to find feline-friendly grooming salons
          </p>
        </div>
      </section>
          ...
          {cities.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {cities.map((city) => (
                <Link
                  key={city.slug}
                  href={`/cat-grooming/${state}/${city.slug}`}
