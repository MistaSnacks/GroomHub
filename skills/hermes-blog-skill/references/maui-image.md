...
5. Keep the verified transparent derivative in the run directory. Pass the [Maui quality gate](maui-quality.md), upload the PNG to Snackbox, and use the returned asset reference in the CMS draft. Download the CMS `originalUrl` and verify its hash; inspect its generated variants as well. Do not write to production `public/`, replace existing artwork, or put an opaque master into a CMS article.

For a new article, select the existing `guideTopic` reference in Snackbox; no code mapping is required. The visitor-facing section is Grooming Guides at `/blog`; existing article routes stay `/blog/<article-slug>`.
